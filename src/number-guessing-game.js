...
function validateInput(input) {
  ...
  if (isNaN(number)) {
    return { isValid: false, message: "That's not a valid number. Please enter a number." };
  }
  if (number < 1 || number > 100) {
    return { isValid: false, message: "Please enter a number between 1 and 100." };
  }
  if (previousGuesses.includes(number)) {
    return { isValid: false, message: "You already guessed that number. Try a new one!" };
  }
...
function processGuess(guess) {
  ...
  if (guess === targetNumber) {
    showFeedback(`You got it! The number was ${targetNumber}!`, "success");
    endGame(true);
  } else if (attemptsLeft === 0) {
    showFeedback("You've run out of attempts!", "error");
    endGame(false);
...
function endGame(won) {
  ...
  if (won) {
    ...
    gameOverMessage.innerHTML = `
            ...
            <p class="text-gray-700">You guessed the number <strong>${targetNumber}</strong> in <strong>${attempts}</strong> attempt${attempts === 1 ? "" : "s"
      }!</p>
        `;
  } else {
    gameOverMessage.innerHTML = `
